...
def download(asset, frequency):
    ...
    if df.empty:
        raise RuntimeError(
            f"No data returned for {ticker}. Check internet/yfinance and try again."
        )
...
def main(asset, frequency="daily"):
    ...
    raw, period, interval = download(asset, frequency)

    raw.to_csv(
        root / f"data/{asset}_{frequency}_raw.csv"
    )
    last_updated = str(raw.index[-1].date())

    (root / f"data/{asset}_{frequency}_last_update.txt").write_text(last_updated)
    ...
    result.to_csv(
        root / f"results/{asset}_{frequency}_prediction_output.csv",
        index=False
    )
    ...
    (
        root / f"results/{asset}_{frequency}_metrics.json"
    ).write_text(
        json.dumps(
            metrics,
            indent=2
    ...
    }, root / f"models/{asset}_{frequency}_lstm.pt")
    (root / f"data/{asset}_{frequency}_last_update.txt").write_text(
        str(df.index[-1])
    )
