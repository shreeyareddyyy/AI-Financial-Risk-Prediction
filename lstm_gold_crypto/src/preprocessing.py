...
def update_features_for_prediction(df):
    """
    Recalculate technical indicators for recursive forecasting.
    Does NOT create Future_Volatility.
    """
    ...
    out["EMA_14"] = close.ewm(span=14, adjust=False).mean()
    ...
    out["Volatility_7"] = out["Return"].rolling(7).std()
    out["Volatility_30"] = out["Return"].rolling(30).std()
